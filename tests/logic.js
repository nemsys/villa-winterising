...
// ---- облак: сливане на две устройства ----
ok(canon({ b: 1, a: [2, { d: 3, c: undefined }] }) === canon({ a: [2, { d: 3 }], b: 1 }), "canon зависи от реда на ключовете");

var loc = { state: { "4": { v: true, t: 100 }, "5": { v: true, t: 300 }, "6": { v: true, t: 50 } },
            notes: { "6": { t: "тук", d: 200 }, "9": { t: "стара тук", d: 10 } }, gone: { "13": 400 },
            items: { "9": { m: 3 } }, layout: { l: "тук" }, ord: 500 };
var rem = { state: { "4": { v: false, t: 200 }, "5": { v: false, t: 100 }, "7": { v: true, t: 1 }, "6": { v: false, t: 50 } },
            notes: { "6": { t: "там", d: 100 }, "9": { t: "нова там", d: 20 }, "13": { t: "отпреди триенето", d: 300 }, "14": { t: "там", d: 5 } },
            gone: { "9": 15 }, items: {}, layout: { l: "там" }, ord: 400 };
var mg = mergeDocs(loc, rem);
ok(mg.state["4"].v === false, "по-новото отмятане от облака не спечели");
ok(mg.state["5"].v === true, "по-новата отметка тук загуби");
ok(mg.state["7"].v === true, "отметка само в облака не дойде");
ok(mg.state["6"].v === false, "при равно време облакът не спечели");
ok(mg.notes["6"].t === "тук", "по-новата бележка тук загуби");
ok(mg.notes["9"].t === "нова там", "бележка, писана след триене, не оцеля");
ok(!mg.notes["13"] && mg.gone["13"] === 400, "изтрита тук бележка се върна от облака");
ok(mg.notes["14"].t === "там", "бележка само в облака не дойде");
ok(mg.gone["9"] === undefined, "остана следа от триене, надписано с нова бележка");
ok(mg.layout.l === "тук" && mg.items["9"].m === 3 && mg.ord === 500, "по-новата наредба тук загуби");
rem.ord = 500;
ok(mergeDocs(loc, rem).layout.l === "там", "при равно време на наредбата облакът не спечели");
ok(sameDoc(mergeDocs(mg, mg), mg), "сливане на документ със себе си го мени");
ok(!sameDoc(mg, rem), "различни документи излизат еднакви");
